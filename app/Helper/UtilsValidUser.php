...
class UtilsValidUser {
    ...
    public static function getUser($data) {
        $role = @$data['user_role'] ?? RolePermission::$ADMIN;
        ...
        if ($data['user_user_type_id'] == 1) {
            ...
            //     'name_en' => @$data['user_name'] ?? 'Noipunno',
        ...
        if (!$user) {
             $u_data = [
                ...
                'role' => $role ?? RolePermission::$ADMIN,
                'name' => @$data['user_name'] ?? 'Noipunno',
                'email' => @$data['user_email'] ?? 'admin@admin.com',
