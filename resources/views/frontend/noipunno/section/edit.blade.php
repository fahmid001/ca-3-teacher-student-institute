...
<h2 class="np-form-title mt-3">সেকশন তথ্য পরিবর্তন করুন </h2>

<section class="section-teacher-add-form mt-3 np-input-form-bg mb-5" id="edit-form">
    <div class="container">
        <form action="{{ route('noipunno.dashboard.section.update', ['id' => @$sectionData->uid]) }}" method="POST" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="row">
                <div class="col-md-4 col-sm-12">
                    <div>
                        ...
                        <div class="input-group">
                            <select class="form-select np-teacher-input" aria-label="Default select example" id="branch1" name="branch_id">
                                <option value="">ব্রাঞ্চ নির্বাচন করুন</option>
                                @foreach ($branchList as $branch)
                                <option value="{{ @$branch->uid }}" {{ old('branch_id', @$branch->uid) == @$sectionData->branch_id ? 'selected' : '' }}>
                                    {{ @$branch->branch_name }}
                                </option>
                                @endforeach
                            </select>
                ...
                <div class="col-md-4 col-sm-12">
                    <div>
                        ...
                        <div class="input-group">
                            <select class="form-select np-teacher-input" aria-label="Default select example" name="version_id" id="version1">
                                <option value="">ভার্সন নির্বাচন করুন</option>
                                @foreach ($versionList as $version)
                                <option value="{{ @$version->uid }}" {{old('version_id', @$version->uid) == @$sectionData->version_id ? 'selected' : ''}}>
                ...
                <div class="col-md-4 col-sm-12">
                    <div>
                        ...
                        <div class="input-group">
                            <select class="form-select np-teacher-input" aria-label="Default select example" name="shift_id" id="shift1">
                                <option value="">শিফট নির্বাচন করুন</option>
                                @foreach ($shiftList as $shift)
                                <option value="{{ @$shift->uid }}" {{ old('shift_id', @$shift->uid) == @$sectionData->shift_id ? 'selected' : '' }}>
                                    {{ @$shift->shift_name }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="input-group">
                            <select class="form-select np-teacher-input" aria-label="Default select example" name="class_id">
                                ...
                                {{-- <option value="6" {{ @$sectionData->class_id == 6 ? 'selected' : ''}}>Six</option>
                                <option value="7" {{ @$sectionData->class_id == 7 ? 'selected' : ''}}>Seven</option> --}}
                                {{-- @foreach ($classList as $class)
                                <option value="{{ $class['class_id'] }}" {{ @$sectionData->class_id == $class['uid'] ? 'selected' : '' }}>
                                {{ $class['name_en'] }}
                                </option>
                                @endforeach --}}

                                @foreach(App\Helper\ClassEnum::values() as $key=>$value)
                                <option value="{{ $key }}" {{ old('class_id', @$sectionData->class_id) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="col-md-4 col-sm-12 mt-5">
                    <div>
                        ...
                        <input type="text" class="form-control np-teacher-input" id="loginId" placeholder="সেকশনের নাম" name="section_name" value="{{ old('section_name',@$sectionData->section_name) }}">
                    </div>
                ...
                <div class="col-md-4 col-sm-12 mt-5">
                    <div>
                        ...
                        <select class="form-select np-teacher-input" aria-label="Default select example" name="section_year">
                            ...
                            <option value="2023" {{ old('section_year',@$sectionData->section_year) == '2023' ? 'selected' : '' }}>2023</option>
                            <option value="2024" {{ old('section_year',@$sectionData->section_year) == '2024' ? 'selected' : '' }}>2024</option>
                        </select>
                ...
                <div class="col-md-4 col-sm-12 mt-5">
                    <div>
                        ...
                        <input type="text" class="form-control np-teacher-input" id="loginId" placeholder="সেকশনের বিস্তারিত তথ্য" name="section_details" value="{{ old('section_details',@$sectionData->section_details) }}">
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12 col-sm-12 d-flex justify-content-start">
                    <button type="submit" class="btn btn-primary np-btn-form-submit mt-3 d-flex align-items-center" style="width: fit-content;border: unset;column-gap: 10px;"> <a class="np-btn-cancel" href="{{ route('noipunno.dashboard.section.add') }}">বাতিল করুন</a>
