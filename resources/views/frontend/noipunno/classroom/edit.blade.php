...
<div class="dashboard-section">
                                ...
                                <div class="content">
                                    ...
                                    <nav aria-label="breadcrumb">
                                        <ol class="breadcrumb np-breadcrumb">
                                            ...
                                            <li class="breadcrumb-item active" aria-current="page">বিষয় শিক্ষক</li>
                                        </ol>
                                    </nav>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <section class="np-teacher-list mt-5 d-none">
                    <div class="container">
                        <div class="row">
                            <div class="col-md-12">
                                <h2 class="title">সেকশন ভিত্তিক বিষয় শিক্ষকের তথ্য</h2>
                                            ...
                                            <table class="table np-table">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td scope="row">{{ @$item->class_teacher->name_en }}</th>
                                                        <td scope="row">
                                                            <div class="action-content">
                                                                {{-- <h2 class="created-date">{{ date('d M, Y', strtotime( @$item->created_at)) }}</h2> --}}
                                                                <a href="{{ route('noipunno.dashboard.classroom.edit',['id'=> $item->uid ]) }}" class="np-route">
...
<h4 class="title mt-5">সেকশন ভিত্তিক বিষয় শিক্ষকের তথ্য সম্পাদনা করুন</h4>
<form action="{{ route('noipunno.dashboard.classroom.update', @$class_room->uid) }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-4 col-sm-12">
                    <div>
                        ...
                        <div class="input-group">
                            <select class="form-select np-teacher-input" name="branch_id" id="branch_id" aria-label="Default select example">
                                {{-- <option value="">Select Branch</option> --}}
                                @foreach ($branches as $item)
                                <option value="{{ $item->uid }}" {{ @$class_room->branch_id == $item->uid ? 'selected' : '' }}>{{ $item->branch_name }}
                                </option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="col-md-4 col-sm-12">
                    <div>
                        ...
                        <div class="input-group">
                            <select class="form-select np-teacher-input" name="version_id" id="version_id" aria-label="Default select example">
                ...
                <div class="col-md-4 col-sm-12">
                    <div>
                        ...
                        <div class="input-group">
                            <select class="form-select np-teacher-input" name="shift_id" id="shift_id" aria-label="Default select example">
                ...
                <div class="col-md-4 col-sm-12 mt-3">
                    <div>
                        ...
                        <div class="input-group">
                            <select class="form-select np-teacher-input" id="class_id" name="class_id" aria-label="Default select example">
                                ...
                                @foreach(App\Helper\ClassEnum::values() as $key=>$value)
                                    <option value="{{ $key }}" {{ @$class_room->class_id == $key ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="col-md-4 col-sm-12 mt-3">
                    <div>
                        <label for="beiin-psid" class="form-label">সেকশন <span class="text-danger">*</span></label>
                        <div class="input-group">
                            <select class="form-select np-teacher-input" id="section_id" name="section_id" aria-label="Default select example">
                    ...
                    <small class="help-block form-text text-danger">{{ $errors->first('section_id') }}</small>
                    @endif
                </div>
                ...
                <div class="col-md-4 col-sm-12 mt-3">
                    <div>
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" value="{{ @$class_room->session_year }}" class="form-control np-teacher-input" name="session_year" id="session_year" readonly>

                            {{-- <select class="form-select np-teacher-input" name="session_year"
                                                aria-label="">
                                                <option value="">Year</option>
                                                @for ($year = date('Y'); $year>'2019'; $year-- )
                                                <option value={{ $year }} {{ $class_room->session_year == $year ? 'selected' : '' }}>{{ $year }}</option>
                            @endfor
                            </select> --}}
                        </div>
                    </div>
                ...
                <div class="col-md-4 col-sm-12 mt-3">
                    <div>
                        <label for="beiin-psid" class="form-label">শ্রেণী শিক্ষক<span class="text-danger">*</span></label>
                        <div class="input-group">
                            {{-- @dd($teachers) --}}
                            <select class="form-select np-teacher-input select2" name="class_teacher_id" aria-label="Default select example">
                                ...
                                <option value="{{ $item->uid }}" {{ @$class_room->class_teacher_id == $item->uid ? 'selected' : '' }}>{{ $item->name_en }} - {{ @$item->pdsid ?? @$item->index_number ?? @$item->caid }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    @if ($errors->has('class_teacher_id'))
                    <small class="help-block form-text text-danger">{{ $errors->first('class_teacher_id') }}</small>
                    @endif
                </div>
            </div>
        </div>
    ...
    <!-- teacher add -->
    <section class="np-teacher-add-form np-input-form-bg">
        <div class="mt-3">
            <div class="container">
                ...
                <form>
                    ...
                    <div class="row">
                        <div class="col-md-12 col-sm-12 d-flex justify-content-start">
                            <button type="submit" class="btn btn-primary np-btn-form-submit mt-3 d-flex align-items-center" style="width: fit-content;border: unset;column-gap: 10px;"> <a class="np-btn-cancel" href="{{ route('noipunno.dashboard.classroom.add') }}">বাতিল করুন</a>
                ...
                if (data.versions) {
                    $.each(data.versions, function(index, category) {
                        $('#version_id').append('<option value ="' + category.uid + '"' + ((
                                category
                                .uid == vselected) ? ('selected') : '') + '>' + category.version_name +
                            '</option>');
                    });
                }
                if (data.shifts) {
                    $.each(data.shifts, function(index, category) {
                        $('#shift_id').append('<option value ="' + category.uid + '"' + ((
                                category
                                .uid == sselected) ? ('selected') : '') + '>' + category.shift_name +
                            '</option>');
                    });
                }
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        <select name="teacher_ids[${subject.uid}]" class="form-select np-teacher-input select2" aria-label="">
                                            ...
                                            @php
                                            $subject_teachers_all = $subject_teachers->where('teacher_id',$teacher->uid);
                                            $json_data  = json_encode($subject_teachers_all->pluck('subject_id')->toArray());
                                            @endphp
